...
void PSIOManager::set_specific_retention(int fileno, bool retain) {
    if (retain) {
        ...
        retained_files_.erase(
            (get_file_path(fileno) + "psi." + pid_ + "." + PSIO::get_default_namespace() + "." + filenum).c_str());
    }
    mirror_to_disk();
}
...
void PSIOManager::write_scratch_file(const std::string& full_path, const std::string& text) {
    ...
    if (!fh) throw PSIEXCEPTION("Unable to write to " + full_path);
...
void PSIOManager::open_file(const std::string& full_path, int fileno) {
    files_[full_path] = true;
    if (specific_retains_.count(fileno) != 0) retained_files_.insert(full_path);
    mirror_to_disk();
}
...
void PSIOManager::build_from_disk() {
    ...
    if (fh == nullptr)
        throw PSIEXCEPTION("PSIOManager cannot get a mirror file handle. Is there a psi.clean file there?\n");
