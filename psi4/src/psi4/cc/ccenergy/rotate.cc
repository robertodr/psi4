...
int CCEnergyWavefunction::rotate()
{
    ...
    */
        ...
        for(int h = 0; h < nirreps; h++) {
            ...
            for(int i = offset[h]+moinfo_.frdocc[h],I=0; i < offset[h]+moinfo_.frdocc[h]+moinfo_.occpi[h]; i++,I++)
                for(int j = offset[h]+moinfo_.frdocc[h],J=0; j < offset[h]+moinfo_.frdocc[h]+moinfo_.occpi[h]; j++,J++)
                    Foo[h][I][J] = fock[i][j];

            for(int a = offset[h]+moinfo_.frdocc[h]+moinfo_.occpi[h],A=0; a < offset[h]+moinfo_.orbspi[h]; a++,A++)
                for(int b = offset[h]+moinfo_.frdocc[h]+moinfo_.occpi[h],B=0; b < offset[h]+moinfo_.orbspi[h]; b++,B++)
                    Fvv[h][A][B] = fock[a][b];
    ...
    */
        ...
        for(int h = 0; h < nirreps; h++) {
            ...
            for(int i = offset[h]+moinfo_.frdocc[h],I=0; i < offset[h]+moinfo_.frdocc[h]+moinfo_.aoccpi[h]; i++,I++)
                for(int j = offset[h]+moinfo_.frdocc[h],J=0; j < offset[h]+moinfo_.frdocc[h]+moinfo_.aoccpi[h]; j++,J++)
                    Foo[h][I][J] = fock_a[i][j];

            for(int a = offset[h]+moinfo_.frdocc[h]+moinfo_.aoccpi[h],A=0; a < offset[h]+moinfo_.orbspi[h]; a++,A++)
                for(int b = offset[h]+moinfo_.frdocc[h]+moinfo_.aoccpi[h],B=0; b < offset[h]+moinfo_.orbspi[h]; b++,B++)
        ...
        for(int h = 0; h < nirreps; h++) {
            ...
            for(int i = offset[h]+moinfo_.frdocc[h],I=0; i < offset[h]+moinfo_.frdocc[h]+moinfo_.boccpi[h]; i++,I++)
                for(int j = offset[h]+moinfo_.frdocc[h],J=0; j < offset[h]+moinfo_.frdocc[h]+moinfo_.boccpi[h]; j++,J++)
                    Foo[h][I][J] = fock_b[i][j];

            for(int a = offset[h]+moinfo_.frdocc[h]+moinfo_.boccpi[h],A=0; a < offset[h]+moinfo_.orbspi[h]; a++,A++)
                for(int b = offset[h]+moinfo_.frdocc[h]+moinfo_.boccpi[h],B=0; b < offset[h]+moinfo_.orbspi[h]; b++,B++)
