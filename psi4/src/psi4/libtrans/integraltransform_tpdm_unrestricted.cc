...
void IntegralTransform::backtransform_tpdm_unrestricted() {
     ...
     * (AA|AA) & (AA|aa) -> (AA|nn)
     */
    global_dpd_->buf4_init(&J1, PSIF_TPDM_PRESORT, 0, DPD_ID("[A>=A]+"), DPD_ID("[A,A]"), DPD_ID("[A>=A]+"),
                           DPD_ID("[A>=A]+"), 0, "MO TPDM (AA|AA)");
    //    global_dpd_->buf4_print(&J1, "outfile", 1);

    global_dpd_->buf4_init(&J2, PSIF_TPDM_PRESORT, 0, DPD_ID("[A>=A]+"), DPD_ID("[a,a]"), DPD_ID("[A>=A]+"),
                           DPD_ID("[a>=a]+"), 0, "MO TPDM (AA|aa)");
    //    global_dpd_->buf4_print(&J2, "outfile", 1);

    global_dpd_->buf4_init(&K, PSIF_TPDM_HALFTRANS, 0, DPD_ID("[A>=A]+"), DPD_ID("[n,n]"), DPD_ID("[A>=A]+"),
                           DPD_ID("[n>=n]+"), 0, "Half-Transformed TPDM (AA|nn)");
    ...
    for (int h = 0; h < nirreps_; h++) {
        if (J1.params->coltot[h] && J1.params->rowtot[h]) {
            ...
            nBuckets =
                static_cast<int>(ceil(static_cast<double>(J1.params->rowtot[h]) / static_cast<double>(rowsPerBucket)));
        ...
        for (int n = 0; n < nBuckets; n++) {
            ...
            for (int pq = 0; pq < thisBucketRows; pq++) {
                for (int Gr = 0; Gr < nirreps_; Gr++) {
                    ...
                    if (nrows && ncols && nlinks)
                        C_DGEMM('n', 't', nrows, ncols, nlinks, 1.0, &J1.matrix[h][pq][rs], nlinks, pca[0], ncols, 0.0,
                                TMP[0], nso_);
            ...
            for (int pq = 0; pq < thisBucketRows; pq++) {
                for (int Gr = 0; Gr < nirreps_; Gr++) {
                    ...
                    if (nrows && ncols && nlinks)
                        C_DGEMM('n', 't', nrows, ncols, nlinks, 1.0, &J2.matrix[h][pq][rs], nlinks, pcb[0], ncols, 0.0,
                                TMP[0], nso_);
        ...
        for (int n = 0; n < nBuckets; n++) {
            ...
            for (int pq = 0; pq < thisBucketRows; pq++) {
                for (int Gr = 0; Gr < nirreps_; Gr++) {
                    ...
                    double **pcb = cb->pointer(Gs);
                    if (nrows && ncols && nlinks)
                        C_DGEMM('n', 't', nrows, ncols, nlinks, 1.0, &J1.matrix[h][pq][rs], nlinks, pcb[0], ncols, 0.0,
                                TMP[0], nso_);
     ...
     * (nn|AA) & (nn|aa) -> (nn|nn)
     */
    global_dpd_->buf4_init(&J1, PSIF_TPDM_HALFTRANS, 0, DPD_ID("[n>=n]+"), DPD_ID("[A,A]"), DPD_ID("[n>=n]+"),
                           DPD_ID("[A>=A]+"), 0, "Half-Transformed TPDM (nn|AA)");
    global_dpd_->buf4_init(&J2, PSIF_TPDM_HALFTRANS, 0, DPD_ID("[n>=n]+"), DPD_ID("[a,a]"), DPD_ID("[n>=n]+"),
                           DPD_ID("[a>=a]+"), 0, "Half-Transformed TPDM (nn|aa)");
    global_dpd_->buf4_init(&K, PSIF_AO_TPDM, 0, DPD_ID("[n>=n]+"), DPD_ID("[n,n]"), DPD_ID("[n>=n]+"),
                           DPD_ID("[n>=n]+"), 0, "SO Basis TPDM (nn|nn)");

    for (int h = 0; h < nirreps_; h++) {
        if (J1.params->coltot[h] && J1.params->rowtot[h]) {
            memFree = static_cast<size_t>(dpd_memfree() - J1.params->coltot[h] - K.params->coltot[h]);
            rowsPerBucket = memFree / (2 * J1.params->coltot[h]);
            if (rowsPerBucket > J1.params->rowtot[h]) rowsPerBucket = static_cast<size_t>(J1.params->rowtot[h]);
            nBuckets =
                static_cast<int>(ceil(static_cast<double>(J1.params->rowtot[h]) / static_cast<double>(rowsPerBucket)));
            rowsLeft = static_cast<size_t>(J1.params->rowtot[h] % rowsPerBucket);
        } else {
