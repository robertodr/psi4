...
void dx_write(std::shared_ptr<Wavefunction> wfn, Options &options, double **D) {
    ...
    printer->Printf("\n");
    printer->Printf("end");
    auto printer2 = std::make_shared<PsiOutStream>("molecule.xyz");

    printer2->Printf("%d\n", molecule->natom());
    printer2->Printf("Initial atomic coordinates\n");
    for (int i = 0; i < molecule->natom(); i++) {
        printer2->Printf("%2s  ", molecule->symbol(i).c_str());
        printer2->Printf("  %9.6f  %9.6f  %9.6f\n", molecule->x(i) * pc_bohr2angstroms,
                         molecule->y(i) * pc_bohr2angstroms, molecule->z(i) * pc_bohr2angstroms);
    }
...
void compute_delta(double **delta, double x, double y, double z) {
    ...
    for (i = 0; i < nmo; i++)
        for (j = 0; j < nmo; j++) delta[i][j] = phi_mo[i] * phi_mo[j];
