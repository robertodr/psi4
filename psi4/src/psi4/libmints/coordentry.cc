...
void CoordEntry::set_basisset(const std::string& name, const std::string& type) { basissets_[type] = name; }
...
void CoordEntry::set_shell(const std::string& name, const std::string& type) { shells_[type] = name; }
...
CartesianEntry::CartesianEntry(int entry_number, double Z, double charge, double mass, const std::string& symbol,
                               const std::string& label, int A, std::shared_ptr<CoordValue> x,
                               std::shared_ptr<CoordValue> y, std::shared_ptr<CoordValue> z)
...
ZMatrixEntry::ZMatrixEntry(int entry_number, double Z, double charge, double mass, const std::string& symbol,
                           const std::string& label, int A, std::shared_ptr<CoordEntry> rto,
                           std::shared_ptr<CoordValue> rval, std::shared_ptr<CoordEntry> ato,
                           std::shared_ptr<CoordValue> aval, std::shared_ptr<CoordEntry> dto,
...
ZMatrixEntry::ZMatrixEntry(int entry_number, double Z, double charge, double mass, const std::string& symbol,
                           const std::string& label, int A, const std::map<std::string, std::string>& basis,
                           const std::map<std::string, std::string>& shells, std::shared_ptr<CoordEntry> rto,
                           std::shared_ptr<CoordValue> rval, std::shared_ptr<CoordEntry> ato,
                           std::shared_ptr<CoordValue> aval, std::shared_ptr<CoordEntry> dto,
                           std::shared_ptr<CoordValue> dval)
...
void ZMatrixEntry::set_coordinates(double x, double y, double z) {
    ...
    if (rto_ != 0) {
        if (!rto_->is_computed()) throw PSIEXCEPTION("Coordinates have been set in the wrong order");
        rval_->set(r(coordinates_, rto_->compute()));
    }

    if (ato_ != 0) {
        if (!ato_->is_computed()) throw PSIEXCEPTION("Coordinates have been set in the wrong order");
    ...
    if (dto_ != 0) {
        if (!dto_->is_computed()) throw PSIEXCEPTION("Coordinates have been set in the wrong order");
        double val = d(coordinates_, rto_->compute(), ato_->compute(), dto_->compute());
        // Check for NaN, and don't update if we find one
        if (val == val) dval_->set(180.0 * val / M_PI);
    }
...
const Vector3& ZMatrixEntry::compute() {
    ...
    if (rto_ == 0 && ato_ == 0 && dto_ == 0) {
        ...
        for (int xyz = 0; xyz < 3; ++xyz)
            coordinates_[xyz] =
                B[xyz] + r * (eX[xyz] * sinABC * cosABCD + eY[xyz] * sinABC * sinABCD - eCB[xyz] * cosABC);
