...
int CharacterTable::make_table() {
    ...
    for (gi = 0; gi < nirrep_; gi++) {
        ...
        for (gj = 0; gj < nirrep_; gj++) {
            ...
            // is so a unit matrix?
            if (std::fabs(1.0 - so[0][0]) < 1.0e-8 && std::fabs(1.0 - so[1][1]) < 1.0e-8 &&
                std::fabs(1.0 - so[2][2]) < 1.0e-8)
                break;
        }
    ...
    if (sym_bits != bits_) throw PSIEXCEPTION("make_table: Symmetry operators did not match the point group given.");
