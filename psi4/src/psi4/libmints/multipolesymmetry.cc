...
OperatorSymmetry::OperatorSymmetry(int order, std::shared_ptr<Molecule> mol, std::shared_ptr<IntegralFactory> ints,
...
std::string OperatorSymmetry::form_suffix(int x, int y, int z) {
    ...
    if (x) {
        suffix += "x";
        if (x > 1) suffix += std::to_string(x);
    }

    if (y) {
        suffix += "y";
        if (y > 1) suffix += std::to_string(y);
    }

    if (z) {
        suffix += "z";
        if (z > 1) suffix += std::to_string(z);
    }
...
MultipoleSymmetry::MultipoleSymmetry(int order, std::shared_ptr<Molecule> mol, std::shared_ptr<IntegralFactory> ints,
                                     std::shared_ptr<MatrixFactory> mats)
    : order_(order), molecule_(mol), integral_(ints), matrix_(mats) {
    common_init();
}
...
int MultipoleSymmetry::address_of_component(int lx, int ly, int lz) {
    ...
    if (l == 0)
        throw PSIEXCEPTION(
            "MultipoleSymmetry::address_of_component - minimum address too low. "
            "Only dipoles and upwards are addressed");
    if (l > order_)
        throw PSIEXCEPTION(
            "MultipoleSymmetry::address_of_component - angular momentum exceeds the order of "
            "this object");
...
void MultipoleSymmetry::common_init() {
    ...
    for (int l = 1; l <= order_; ++l) {
        ...
        for (int irrep = 0; irrep < nirrep; ++irrep) {
            ...
            for (int xyz = 0; xyz < ncart; ++xyz) {
                if (t[xyz] != 0) component_symmetry_[l_offset + xyz] = irrep;
            }
        }
        l_offset += ncart;
