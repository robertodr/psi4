...
OneBodySOInt::OneBodySOInt(const std::shared_ptr<OneBodyAOInt> &ob, const IntegralFactory *integral)
...
void OneBodySOInt::common_init() {
    b1_ = std::make_shared<SOBasisSet>(ob_->basis1(), integral_);
...
std::shared_ptr<SOBasisSet> OneBodySOInt::basis1() const { return b1_; }

std::shared_ptr<SOBasisSet> OneBodySOInt::basis2() const { return b2_; }

std::shared_ptr<OneBodyAOInt> OneBodySOInt::ob() const { return ob_; }
                        ...
                        for (int jtr = 0; jtr < s2.nfunc; ++jtr) {
                            ...
                            for (int i = 0; i < nchunk; ++i) {
                                ...
                                if (ijirrep == results[i]->symmetry()) {
                                    ...
                                    results[i]->add(ifunc.irrep, b1_->function_within_irrep(ish, isofunc),
                                                    b2_->function_within_irrep(jsh, jsofunc), temp);
                                }
                            }
                        }
                    }
...
void OneBodySOInt::compute_deriv1(std::vector<SharedMatrix> result, const CdSalcList &cdsalcs) {
    // Do not worry about zeroing out result.
...
TwoBodySOInt::TwoBodySOInt(const std::shared_ptr<TwoBodyAOInt> &tb, const std::shared_ptr<IntegralFactory> &integral)
    :
...
void TwoBodySOInt::common_init() {
    ...
    b1_ = std::make_shared<SOBasisSet>(tb_[0]->basis1(), integral_);

    if (tb_[0]->basis1() == tb_[0]->basis2())
        b2_ = b1_;
    else
        b2_ = std::make_shared<SOBasisSet>(tb_[0]->basis2(), integral_);

    if (tb_[0]->basis1() == tb_[0]->basis3())
        b3_ = b1_;
    else
        b3_ = std::make_shared<SOBasisSet>(tb_[0]->basis3(), integral_);
    ...
    if (tb_[0]->deriv() == 1 && cdsalcs_ != 0)
        for (int i = 0; i < nthread_; ++i) buffer_.push_back(new double[size_ * cdsalcs_->ncd()]);
    else {
    ...
    dcd_ = std::make_shared<DCD>(petite1_->group());

    if (cdsalcs_) {
        int ncd = cdsalcs_->ncd();
        for (int n = 0; n < nthread_; ++n) {
            deriv_.push_back(new double *[ncd]);
            for (int i = 0; i < ncd; ++i) deriv_[n][i] = &(buffer_[n][i * size_]);
        }
    }
...
TwoBodySOInt::~TwoBodySOInt() {
    for (int i = 0; i < nthread_; ++i) {
        ...
        //            delete[] temp2_[i];
        //        }
        if (deriv_.size()) delete[] deriv_[i];
    }
}

std::shared_ptr<SOBasisSet> TwoBodySOInt::basis() const { return b1_; }

std::shared_ptr<SOBasisSet> TwoBodySOInt::basis1() const { return b1_; }

std::shared_ptr<SOBasisSet> TwoBodySOInt::basis2() const { return b2_; }
