...
void X2CInt::setup(std::shared_ptr<BasisSet> basis, std::shared_ptr<BasisSet> x2c_basis) {
    ...
    integral_ = std::make_shared<IntegralFactory>(aoBasis_, aoBasis_, aoBasis_, aoBasis_);
...
void X2CInt::diagonalize_dirac_h() {
    ...
    E_LS_Mat = std::make_shared<Vector>("Dirac EigenValues", C_LS_Mat->rowspi());
...
void X2CInt::project() {
    ...
    std::shared_ptr<SOBasisSet> soBasis_contracted =
        std::make_shared<SOBasisSet>(aoBasis_contracted_, integral_contracted);
...
void X2CInt::test_h_FW_plus() {
    ...
    for (int h = 0; h < dMat->nirrep(); ++h) {
        ...
        for (int p = 0; p < maxp; ++p) {
            ...
            //            outfile->Printf("\n %d,%2d = %20.12f %20.12f
            //            %20.12f",h,p,eval_dirac,eval_x2c,eval_dirac-eval_x2c);
        }
    }
