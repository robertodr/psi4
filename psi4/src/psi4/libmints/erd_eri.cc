...
void C_ERD__GENER_ERI_BATCH(const F_INT &imax, const F_INT &zmax, const F_INT &nalpha, const F_INT &ncoeff,
                            const F_INT &ncsum, const F_INT &ncgto1, const F_INT &ncgto2, const F_INT &ncgto3,
                            const F_INT &ncgto4, const F_INT &npgto1, const F_INT &npgto2, const F_INT &npgto3,
                            const F_INT &npgto4, const F_INT &shell1, const F_INT &shell2, const F_INT &shell3,
                            const F_INT &shell4, const double &x1, const double &y1, const double &z1, const double &x2,
                            const double &y2, const double &z2, const double &x3, const double &y3, const double &z3,
                            const double &x4, const double &y4, const double &z4, const double *alpha, const double *cc,
                            const F_INT *ccbeg, const F_INT *ccend, const F_BOOL &spheric, const F_BOOL &screen,
                            F_INT *icore, F_INT &nbatch, F_INT &nfirst, double *zcore);
void C_ERD__MEMORY_ERI_BATCH(const F_INT &nalpha, const F_INT &ncoeff, const F_INT &ncgto1, const F_INT &ncgto2,
                             const F_INT &ncgto3, const F_INT &ncgto4, const F_INT &npgto1, const F_INT &npgto2,
                             const F_INT &npgto3, const F_INT &npgto4, const F_INT &shell1, const F_INT &shell2,
                             const F_INT &shell3, const F_INT &shell4, const double &x1, const double &y1,
                             const double &z1, const double &x2, const double &y2, const double &z2, const double &x3,
                             const double &y3, const double &z3, const double &x4, const double &y4, const double &z4,
                             const double *alpha, const double *cc, const F_BOOL &spheric, F_INT &imin, F_INT &iopt,
                             F_INT &zmin, F_INT &zopt);
}

namespace psi {

ERDTwoElectronInt::ERDTwoElectronInt(const IntegralFactory *integral, int deriv, bool use_shell_pairs)
    : TwoBodyAOInt(integral, deriv), d_buffer_size_(0L), i_buffer_size_(0L), buffer_offset_(0L) {
    bs1_ = original_bs1_;
    bs2_ = original_bs2_;
    bs3_ = original_bs3_;
    bs4_ = original_bs4_;
    ...
    has_puream_ = bs1_->has_puream() || bs2_->has_puream() || bs3_->has_puream() || bs4_->has_puream();

    spheric_ = 0;  // bs1_->has_puream() && bs2_->has_puream() &&
                   // bs3_->has_puream() && bs4_->has_puream();

    size_t max_cart = INT_NCART(basis1()->max_am()) * INT_NCART(basis2()->max_am()) * INT_NCART(basis3()->max_am()) *
    ...
    size_t max_nprim = basis1()->max_nprimitive() + basis2()->max_nprimitive() + basis3()->max_nprimitive() +
...
void ERDTwoElectronInt::compute_scratch_size() {
    ...
    C_ERD__MEMORY_ERI_BATCH(npgto, npgto, ncgto1, ncgto2, ncgto3, ncgto4, npgto1, npgto2, npgto3, npgto4, am1, am2, am3,
                            am4, x1, y1, z1, x2, y2, z2, x3, y3, z3, x4, y4, z4, alpha_, cc_, spheric_, imin, iopt,
                            zmin, zopt);
#if DEBUG
...
void ERDTwoElectronInt::normalize_basis() {
    ...
    for (int shell = 0; shell < original_bs1_->nshell(); ++shell) {
        ...
        double prefac = 1.0;
        if (L > 1) prefac = pow(2.0, 2 * L) / df[2 * L];
...
size_t ERDTwoElectronInt::compute_shell(int shell_i, int shell_j, int shell_k, int shell_l) {
    ...
    outfile->Printf("\n\nShell (%2d %2d | %2d %2d) - center (%2d %2d | %2d %2d) - angular momentum (%d %d | %d %d)\n",
                    shell_i, shell_j, shell_k, shell_l, bs1_->shell(shell_i).ncenter(), bs2_->shell(shell_j).ncenter(),
                    bs3_->shell(shell_k).ncenter(), bs4_->shell(shell_l).ncenter(), am1, am2, am3, am4);
    ...
    for (int n = 0; n < npgto; ++n) outfile->Printf("%14.10f ", cc_[n]);
    outfile->Printf("\n");
    outfile->Printf("Exponents:    ");
    for (int n = 0; n < npgto; ++n) outfile->Printf("%14.10f ", alpha_[n]);
    outfile->Printf("\n");
#endif
    ...
    C_ERD__GENER_ERI_BATCH(i_buffer_size_, d_buffer_size_, npgto, npgto, ncgto, ncgto4, ncgto3, ncgto2, ncgto1, npgto4,
                           npgto3, npgto2, npgto1, am4, am3, am2, am1, x4, y4, z4, x3, y3, z3, x2, y2, z2, x1, y1, z1,
                           alpha_, cc_, ccbeg_, ccend_, spheric_, screen_, iscratch_, nbatch, buffer_offset_,
                           dscratch_);
