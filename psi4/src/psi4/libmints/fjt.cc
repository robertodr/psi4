...
Taylor_Fjt::Taylor_Fjt(size_t mmax, double accuracy)
    : cutoff_(accuracy), interp_order_(TAYLOR_INTERPOLATION_ORDER), F_(new double[mmax + 1]) {
    ...
    error <= delT^n/(n+1)!
   ---------------------------------------*/
    delT_ = 2.0 * std::pow(cutoff_ * fac[interp_order_ + 1], 1.0 / interp_order_);
    oodelT_ = 1.0 / delT_;
    ...
    for (int m = max_m_; m >= 0; --m) {
     ...
     ------------------------------------------*/
        ...
        do {
            ...
            /* f(T) has 2 roots and has a maximum in between. If f'(T) > 0 we are to the left of the hump. Make a big
             * step to the right. */
            if (dfuncdT > 0.0) {
                T_new *= 2.0;
            } else {
 ...
 */
double* Taylor_Fjt::values(int l, double T) {
...
#error upward recursion cannot be used with taylor interpolation
        ...
        for (int i = 0; i < l; ++i) pow_two_T_to_minusjp05 *= X * X;
 ...
 * The result is placed in the global intermediate int_fjttable.
 */
double* FJT::values(int J, double wval) {
    ...
    if (itable < TABLESIZE) {
        ...
        /* Compute fjt for J. */
        int_fjttable[J] = (((((coef6 * gtable[J + 6][itable] * wdif + coef5 * gtable[J + 5][itable]) * wdif +
                              coef4 * gtable[J + 4][itable]) *
                                 wdif +
                             coef3 * gtable[J + 3][itable]) *
                                wdif +
                            coef2 * gtable[J + 2][itable]) *
                               wdif -
                           gtable[J + 1][itable]) *
                              wdif +
                          gtable[J][itable];
...
GaussianFundamental::~GaussianFundamental() { delete[] value_; }
...
double* F12Fundamental::values(int J, double T) {
    ...
    for (int n = 0; n <= J; ++n) value_[n] = 0.0;
...
F12G12Fundamental::F12G12Fundamental(std::shared_ptr<CorrelationFactor> cf, int max) : GaussianFundamental(cf, max) {
    Fm_ = std::make_shared<FJT>(max);
}
...
ErfFundamental::ErfFundamental(double omega, int max) : GaussianFundamental(std::shared_ptr<CorrelationFactor>(), max) {
    omega_ = omega;
    rho_ = 0;
    boys_ = std::make_shared<FJT>(max);
}
...
double* ErfComplementFundamental::values(int J, double T) {
    ...
    for (int n = 0; n <= J; ++n) value_[n] = Fvals[n];
