...
enum Groups {
    ...
    D2h = SymmOps::E | SymmOps::C2_x | SymmOps::C2_y | SymmOps::C2_z | SymmOps::i | SymmOps::Sigma_xy |
...
class SymmetryOperation {
   ...
   public:
    ...
    const std::array<std::array<double, 3>, 3>& matrix() { return d; }
...
class IrreducibleRepresentation {
    ...
    */
    ...
    /** Returns the character for the i'th symmetry operation of the point
     group. */
    double character(int i) const { return complex_ ? 0.5 * rep[i].trace() : rep[i].trace(); }
...
class CharacterTable {
   ...
   public:
    ...
    IrreducibleRepresentation& gamma(int i) const { return gamma_[i]; }
    /// Returns the i'th symmetry operation.
    SymmetryOperation& symm_operation(int i) const { return symop[i]; }
